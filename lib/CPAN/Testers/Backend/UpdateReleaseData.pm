package CPAN::Testers::Backend::UpdateReleaseData;
our $VERSION = '0.006';
# ABSTRACT: Update the release information
...
data (via L<CPAN::Testers::Schema::Result::ReleaseStat>).
...
sub run( $self, @args ) {
    ...
    my $release_rs = $self->schema->resultset('ReleaseStat');
    ...
    $stats_rs = $stats_rs->search({ id => { '>' => $max_release_data_id }}, { order_by => 'id', prefetch => 'upload'});
    my $written = 0;
    my $batch_size = 10000;
    my @batch;
    while (my $row = $stats_rs->next) {
        ...
        push @batch, {
            %data{qw(dist version id guid uploadid)},
            # pass/fail/na/unknown => 1
            lc $data{state} => 1,
            # distmat
            distmat => $data{version} =~ /_/ || $data{version} =~ /-TRIAL$/ ? 2 : 1,
            # oncpan
            oncpan => !$row->upload->type || $row->upload->type ne 'backpan' ? 1 : 2,
            # perlmat (devel or stable)
            perlmat => $data{perl} =~ /^5.(7|9|[1-9][13579])/ ? 2 : 1,
            # patched (pre-release)
            patched => $data{perl} =~ /RC\d+|patch/ ? 2 : 1,
        };
        if (@batch >= $batch_size) {
