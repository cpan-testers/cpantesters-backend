...
sub update {
    ...
    my %cpan;
    $self->_log( "pre-caching existing uploads" );
    for my $row ( $db->get_query( 'hash', 'SELECT * FROM uploads' ) ) {
        $found{ $row->{author} }{ $row->{dist} }{ $row->{version} } = [ $row ];
        if ( $row->{type} eq 'cpan' ) {
            $cpan{ $row->{filename} } = $row;
        }
    }
...
sub _parse_archive {
    ...
    if ( !$found{ $cpanid }{ $name }{ $version } ) {
        $found{ $cpanid }{ $name }{ $version } = [ $db->get_query('hash',$phrasebook{'FindDistVersion'},$cpanid,$name,$version) ];
    }
    my @rows = @{ $found{ $cpanid }{ $name }{ $version } };
    if(@rows) {
        if($type ne $rows[0]{type}) {
        ...
        if ( $type eq 'cpan' ) {
            my %upload = (
                ...
                author => $cpanid,
	        filename => $filename,
	        released => DateTime->from_epoch( epoch => $date )->iso8601 . 'Z',
	    );
            $self->_log( "Sending WS update: $type, $name, $version, $cpanid, $upload{released}" );
	    Mojo::UserAgent->new->post( 'http://127.0.0.1:9000/pub/upload/dist/' . $upload{dist}, json => \%upload );
	    Mojo::UserAgent->new->post( 'http://127.0.0.1:9000/pub/upload/author/' . $upload{author}, json => \%upload );
        }
